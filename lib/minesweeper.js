...
easy.addEventListener("click", () => {
  BOMBS = 7;
  amountOfBombInGame.innerText = `${BOMBS}`;
...
const checkGameWon = () => {
  if (!isGameLost) {
    ...
    if (JSON.stringify(remainingBoxesID.sort()) === JSON.stringify(bombList.sort())) {
      revealAll();
      const win = document.getElementById("win-or-lose");
      const body = document.querySelector("body")
      body.style.backgroundColor = "#81ff7c"
      win.innerText = "Congratulation, you won ! 🥳"
      stopTimer();
...
const checkGameLost = () => {
  if (isGameLost) {
    ...
    const win = document.getElementById("win-or-lose");
    const body = document.querySelector("body")
    body.style.backgroundColor = "#ff7c7c"
    win.innerText = "You touched a mine.. You lose 😞"
    stopTimer();
  }
}
